...
@pytest.mark.parametrize(
    "sample_p, sample_q, expected_frequencies_p, expected_frequencies_q",
    [
        (
            multinomial_sample_p_1,
            multinomial_sample_q_1,
            expected_frequencies_p_1,
            expected_frequencies_q_1,
        ),
        (
            multinomial_sample_p_2,
            multinomial_sample_q_2,
            expected_frequencies_p_2,
            expected_frequencies_q_2,
        ),
    ],
)
def test_construct_frequencies(
    sample_p: np.ndarray,
    sample_q: np.ndarray,
...
def test_construct_frequencies_error_q_zero_and_p_nonzero():
    ...
    with pytest.raises(ValueError):
        _combined_frequencies_p, _combined_frequencies_q = (
            _construct_frequencies_for_two_samples(
                sorted_p_realizations=unique_p,
                sorted_q_realizations=unique_q,
                sorted_q_frequencies=frequencies_q,
...
def _discrete_relative_entropy_slow(
    sample_p: np.ndarray, sample_q: np.ndarray, log_fun: tp.Callable = np.log
):
    combined_sample = np.hstack((sample_p, sample_q))
    unique_combined = np.unique(combined_sample)
    ...
    unique_q, counts_q = np.unique(sample_q, return_counts=True)
    frequencies_q = counts_q / len(sample_q)
    realization_to_frequency_dict_q = dict(zip(unique_q, frequencies_q, strict=False))

    unique_p, counts_p = np.unique(sample_p, return_counts=True)
    frequencies_p = counts_p / len(sample_p)
    realization_to_frequency_dict_p = dict(zip(unique_p, frequencies_p, strict=False))

    combined_frequencies_q = np.array(
        [
            realization_to_frequency_dict_q.get(realization, 0.0)
            for realization in unique_combined
        ]
    )

    combined_frequencies_p = np.array(
        [
            realization_to_frequency_dict_p.get(realization, 0.0)
            for realization in unique_combined
        ]
    )
    ...
    return sp.stats.entropy(
        pk=combined_frequencies_p, qk=combined_frequencies_q, base=base
    )
...
@pytest.mark.parametrize(
    "sample_p, sample_q",
    (
        (multinomial_sample_p_1, multinomial_sample_q_1),
...
def test_compare_slow_and_fast_implementations_of_relative_entropy(
    ...
    relative_entropy_from_slow_calculation = _discrete_relative_entropy_slow(
        sample_p=sample_p, sample_q=sample_q, log_fun=log_fun
    )

    relative_entropy_from_fast_calculation = discrete_relative_entropy(
        sample_p=sample_p, sample_q=sample_q, base=base
    )

    assert np.isclose(
        relative_entropy_from_slow_calculation, relative_entropy_from_fast_calculation
    )
...
def test_get_index_for_combination(
    combination: np.ndarray, index: int, sample_x: np.ndarray, sample_y: np.ndarray
):
    unique_combinations, _counts = (
        _construct_unique_combinations_and_counts_from_two_samples(sample_x, sample_y)
    )

    assert index == _get_index_for_combination(
...
def test_get_count_for_combination(
    combination: np.ndarray, count: int, sample_x: np.ndarray, sample_y: np.ndarray
):
    unique_combinations, counts = (
        _construct_unique_combinations_and_counts_from_two_samples(sample_x, sample_y)
    )

    assert count == _get_count_for_combination(
        combination=combination, unique_combinations=unique_combinations, counts=counts
    )


@pytest.mark.parametrize("value, index", [(1, 0), (2, 1), (3, 2)])
def test_get_index_for_value(value: numbers.Number, index: int, sample_x: np.ndarray):
...
def test_get_count_for_value(value: numbers.Number, count: int, sample_x: np.ndarray):
    unique_values, counts = np.unique(sample_x, return_counts=True)

    assert count == _get_count_for_value(
        value, unique_values=unique_values, counts=counts
    )
...
def test_symmetry_of_mutual_information(sample_x, sample_y):
    assert discrete_mutual_information(
        sample_x, sample_y
...
def test_discrete_conditional_entropy(sample_x: np.ndarray, sample_y: np.ndarray):
    ...
    conditional_entropy_of_y_given_x = discrete_conditional_entropy_of_y_given_x(
        sample_x=sample_x, sample_y=sample_y
    )

    conditional_entropy_of_x_given_y = discrete_conditional_entropy_of_y_given_x(
        sample_x=sample_y, sample_y=sample_x
    )

    assert np.isclose(
        entropy_x - conditional_entropy_of_x_given_y,
...
def test_discrete_mutual_information_and_conditional_entropy(
