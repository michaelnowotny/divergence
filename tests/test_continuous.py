...
def entropy_of_normal_distribution(
    sigma: float, log_fun: tp.Callable = np.log
) -> float:
    return 0.5 * (1.0 + log_fun(2 * np.pi * sigma**2))


def relative_entropy_between_two_normal_distributions(
    mu_1: float,
    sigma_1: float,
    mu_2: float,
    sigma_2: float,
    ...
    return ((mu_1 - mu_2) ** 2 + sigma_1**2 - sigma_2**2) / (2 * sigma_2**2) + log_fun(
        sigma_2 / sigma_1
    )


def cross_entropy_between_two_normal_distributions(
    mu_1: float,
    sigma_1: float,
    mu_2: float,
    sigma_2: float,
    ...
    return entropy_of_normal_distribution(
        sigma_1, log_fun=log_fun
    ) + relative_entropy_between_two_normal_distributions(
        mu_1=mu_1, sigma_1=sigma_1, mu_2=mu_2, sigma_2=sigma_2, log_fun=log_fun
    )
...
def generate_normal_sample(
    mu: float, sigma: float, n: int, antithetic: bool = False
) -> np.ndarray:
    z = np.random.randn(n)
    if antithetic:
        z = np.hstack((z, -z))
...
def test_entropy(sigma: float, sample: np.ndarray, base: float = np.e):
    ...
    assert np.isclose(
        entropy_from_samples(sample, base=base, discrete=False),
...
def test_cross_entropy(base: float = np.e):
    ...
    assert np.isclose(
        cross_entropy_from_samples(sample_p, sample_q, base=base, discrete=False),
...
def test_relative_entropy(base: float = np.e):
    ...
    assert np.isclose(
        relative_entropy_from_samples(sample_p, sample_q, base=base, discrete=False),
        relative_entropy_between_two_normal_distributions(
            mu_p, sigma_p, mu_q, sigma_q, log_fun=log_fun
        ),
        rtol=1e-1,
...
def test_mutual_information(mutual_information_from_bivariate_normal_samples):
    theoretical_mutual_information = (
        mutual_information_for_bivariate_normal_distribution(rho)
    )
